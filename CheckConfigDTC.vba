...
'---- Start the loop
    For D = 2 To FailureRangeD.Cells.Count
        ...
        Select Case FailureType
            Case "Missing Frame" 'TODO for the last two, consider
                ...
                ConfigurationDIDListed = Split(configurationDIDBrut, vbLf) 'format case1 vbLf Or vbLf case2. In the same line all conditions in AND (but also cases with different values of same DID in OR
                Dim i As Integer
                Dim singleParam() As String
                Dim j As Integer
                Dim BoolAnd As Boolean
                Dim values As String
                For i = 0 To UBound(ConfigurationDIDListed)
                    If ConfigurationDIDListed(i) <> "OR" Then ' if the line contains only OR will be skipped
                        singleParam = Split(ConfigurationDIDListed(i), "&")
                        BoolAnd = True
                        For j = 0 To UBound(singleParam)
                            Debug.Print (singleParam(j))

                        'For j = 0 To UBound(singleParam)
                        '    Dim values As String
                        '    Dim BoolOr As Boolean
                        '    BoolOr = False
                        '    ParamName = Left(singleParam(j), InStr(singleParam(j), "=") - 2) 'Public
                        '    Dim singleValue() As String
                        '    singleValue = Split(singleParam(j), "Or")
                        '    Dim k As Integer
                        '    Dim tempMask As Integer
                        '    For k = 0 To UBound(singleValue)
                        '        tempMask = maskDIDbin(ParamName, Right(singleValue(k), 2)) 'Format number without : meaning
                        '        FileOut.WriteLine (CanoeReadDID(DID))
                        '        FileOut.WriteLine (CanoeWriteComment("check this value, and return boolean: " + tempMask))
                        '        'if one of these is true -> BoolOr = True
                        '        FileOut.WriteLine (CanoeWriteComment("if one of these is true -> BoolOr = True"))
                        '    Next k
                        '    'BoolAnd = BoolOr
                            FileOut.WriteLine (CanoeWriteComment("at end of each cycle param, BoolAnd = BoolOr"))
                        Next j
                        ...
                        'Here it should have decided if the active config is TRUE or FALSE
                        'If (configCase) Then
                        '    'execute positive case -> cut frame and check DTC is raised
                        'Else
                        '    'execute negative case -> cut frame and check DTC is not raised
                        'End If
                    End If
                Next i
                '==================== CONFIG ==================
                ...
                Set MyFSO = Nothing
                Set FileOut = Nothing
            Case Else '"Unavailable", "Out Of Range", "Not Used", "NotUsed/OutOfRange"FailureType = "", NA, "_", and whatever else not listed in previous case
                ScriptNameSpecific = "."
        End Select
